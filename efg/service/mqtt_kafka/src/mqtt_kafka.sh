...
do
  if [ ! -z "$line" ]; then 
    ...
    epoch=$(date +%s)
    json='{"date":'$epoch',"id":"'$id'","i":'$intensity'}'
...
#     kafkacat "${json}" -P -b $EVENT_STREAM_BROKER_URL -X api.version.request=true -X security.protocol=sasl_ssl -X sasl.mechanisms=PLAIN -X sasl.username=${EVENT_STREAM_API_KEY:0:16} -X sasl.password=${EVENT_STREAM_API_KEY:16} -t "$EVENT_STREAM_EFG_TOPIC"
  fi
done
